...
export default function OAuth() {
  ...
  const getAuthUrl = async () => {
    ...
    try {
      ...
      setMessage({
        type: "error",
        text: err.response?.data?.detail || "获取认证链接失败",
      });
    } finally {
      setLoading(false);
    }
  ...
  // 成功后保存凭证信息，用于导出
  const [successCredential, setSuccessCredential] = useState(null);
  ...
  const submitCallbackUrl = async () => {
    ...
    try {
      ...
      const donateText = res.data.is_public
        ? "（已上传到公共池 🎉）"
        : "（私有凭证）";
      setMessage({
        type: "success",
        text: `凭证获取成功！邮箱: ${res.data.email} ${donateText}`,
      });
      // 保存成功的凭证信息，用于导出
      setSuccessCredential({
        id: res.data.credential_id,
  ...
  // 导出凭证功能
  const exportCredential = async () => {
    if (!successCredential?.id) return;
    setExporting(true);
    try {
      const res = await api.get(
        `/api/auth/credentials/${successCredential.id}/export`,
      );
      ...
      a.href = url;
      a.download = `credential_${successCredential.email || successCredential.id}.json`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
      setMessage({ type: "success", text: "凭证已导出！请妥善保管。" });
    } catch (err) {
      setMessage({
        type: "error",
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-dark-800 rounded-2xl max-w-md w-full p-6 border border-dark-600">
            ...
            <div className="space-y-4 text-gray-300">
              ...
              <p>
                授权后，浏览器会打开一个以{" "}
                <span className="text-purple-400 font-mono">localhost</span>{" "}
                开头的页面，该页面显示
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
        ...
        {message.text && (
          ...
          >
            <div className="flex items-center justify-between gap-4">
              ...
              {/* 成功后显示导出按钮 */}
              {message.type === "success" && successCredential?.id && (
                <button
                  onClick={exportCredential}
                  disabled={exporting}
                  className="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white rounded-lg text-sm font-medium whitespace-nowrap"
                >
                  {exporting ? (
        ...
        <div className="card p-6">
          ...
          <p className="text-gray-400 text-sm mb-4">
            请在完成Google授权后，从打开的显示
            <span className="text-red-400">无法访问</span>的页面的地址栏中
            <span className="text-amber-400 font-medium">完整复制整个网址</span>
            并粘贴到下方。
          </p>
        ...
        <div className="card p-6">
          <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
            <span className="w-7 h-7 rounded-full bg-green-600 text-white text-sm flex items-center justify-center">
              3
            </span>
            提交并生成凭证
          </h2>
