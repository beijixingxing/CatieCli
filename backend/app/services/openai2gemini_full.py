...
从 gcli2api 完整复制，负责 OpenAI 格式与 Gemini 格式的双向转换
"""
...
# 尝试导入 pypinyin，如果不存在则使用简单替代
try:
    from pypinyin import Style, lazy_pinyin
except ImportError:
    def lazy_pinyin(char, style=None):
        return [char]
    class Style:
        NORMAL = None
...
# thoughtSignature 相关函数的简单实现
def encode_tool_id_with_signature(tool_id: str, signature: str = "") -> str:
    if signature:
        return f"{tool_id}:{signature}"
    return tool_id

def decode_tool_id_and_signature(encoded_id: str) -> Tuple[str, str]:
    if ":" in encoded_id:
